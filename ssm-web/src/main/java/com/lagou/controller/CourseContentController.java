...
public class CourseContentController {
    ...
    /*
       新增&更新课时信息
    */
    @RequestMapping("/saveOrUpdateLession")
    public ResponseResult saveOrUpdateLession(@RequestBody CourseLesson courseLesson){

        // 判断是否携带了章节ID
        if(courseLesson.getId() == null){
            // 新增
            courseConentService.saveLesson(courseLesson);
            return  new ResponseResult(true,200,"新增课时成功",null);
        }else {
            // 更新
            courseConentService.updateLession(courseLesson);
    ...
    /*
        修改课时状态
     */
    @RequestMapping("/updateLessionStatus")
    public ResponseResult updateLessionStatus(int id,int status){

        courseConentService.updateLessionStatus(id,status);
